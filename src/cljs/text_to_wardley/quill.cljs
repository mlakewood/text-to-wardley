    ...
    (r/create-class
     {:component-did-mount
      (fn [component]
        ...
        (.on ^HTMLInputElement @this "text-change"
             (fn [delta old-delta source]
               (on-change-fn source (value))))

        (if (= selection nil)
          (.setSelection ^HTMLInputElement @this nil)
          (.setSelection ^HTMLInputElement @this (first selection) (second selection) "api")))
