(ns text-to-wardley.slatejs.views
  (:require
...
;; all parts come from -> https://github.com/jhund/re-frame-and-reagent-and-slatejs
  ...
  (let [section-data @(re-frame/subscribe [:text-to-wardley.section/sub-data section-key])
        ...
        on-change-fn (fn [change-or-editor]
                       (let [new-value (.-value change-or-editor)]
                         ...
                         ;; app-db.
                         (re-frame/dispatch [:text-to-wardley.slatejs/evt-editor-value-changed section-key new-value])))
    ...
    (reagent/create-class
     {:display-name "slatejs-editor-inner"
      ...
      :reagent-render (fn [_]
                        [:> slate/Editor
                         {:auto-focus   true
                          :class-name   "text-to-wardley-slatejs-editor"
                          :id           (str "text-to-wardley-slatejs-editor-instance-" section-key)
...
(defn toolbar
  ...
  [:div.text-to-wardley-slatejs-toolbar
   [:h3 "Shared toolbar"]
