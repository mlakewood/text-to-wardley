...
* [`src/cljs/text_to_wardley/`](src/cljs/text_to_wardley/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/text_to_wardley/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/text_to_wardley/`](test/cljs/text_to_wardley/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `text-to-wardley` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/text_to_wardley/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns text-to-wardley.example
  (:require [text-to-wardley.config :as config])
